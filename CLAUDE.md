...
| **Sweep covers 4 metrics + logs, bypasses tool budget** | `sweep_probes_node` queries `probe_up`, `probe_latency`, `cpu_usage`, `memory_usage` for every service in `affected_services`, PLUS a per-service `search_logs` with crash-pattern query. 6 services × 4 metrics + 6 log calls = 30 queries per investigation. These do NOT decrement `tool_calls_remaining` — the LLM keeps its full 10-call budget. Each sweep evidence entry carries `args["critical"] = bool` set from the pre-truncation tool result. |
