...
| **Fine-tuning requires new log-enriched baseline** | The existing 24h baseline has metrics only. Fine-tuning on OTel Demo requires combined log+metric feature vectors from `FeatureEngineer.build_sequence()`. A 4-8h baseline with Promtail shipping logs is needed before fine-tuning. See PROGRESS.md Week 6.5 tasks. |
